...
export class CurtainStage extends DelegateStage {
    ...
    render(ctx: Context, shiftX: number = 0, shiftY: number = 0) {
        ...
        ctx.fillRect(0, 0, GameScreen.it.getWidth(), GameScreen.it.getHeight() * per, `black`, null);
        ctx.fillText(this.baseStage.name, GameScreen.it.getWidth() / 2, GameScreen.it.getHeight() * (per - 0.5), 0.5, 0.5, 40, `white`, null);
    }
}
